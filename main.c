...
#define USB_VENDOR_ID	    5824      /* USB vendor ID used by the device
                                         * 0x0483 is STMs ID
                                         */
#define USB_PRODUCT_ID	    1500     /* USB product ID used by the device */
...
#define USB_BUF_SIZE	        64        /* Connection timeout (in ms) */
...
int main(int argc, char **argv)
{
	...
	char *data = (char *) malloc(USB_BUF_SIZE * sizeof(char));

	memset(data, 'Y', USB_BUF_SIZE);
	...
	while (tries <= max_tries) {
		...
		if (tries % 2 == 0)
		{
			...
			printf("LED OFF\n");
			memset(data, 'N', USB_BUF_SIZE);
		}

		usb_write(data, USB_BUF_SIZE);
